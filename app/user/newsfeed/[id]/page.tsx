...
const SingleNewsFeed = () => {
  const { id } = useParams();
  const { data, isLoading } = useGetSingleNewsQuery<any>(id);
  ...
  return (
    <MaxWidthContainer>
      ...
      <>
        {isLoading ? (
          <Skeleton className="w-[100px] mt-5 ml-4 h-[40px] bg-gray-700 rounded-md" />
        ) : (
          <Link
            href={"/user/newsfeed"}
            className="text-white w-[130px] rounded-lg mt-5 capitalize flex gap-2 px-2 py-1 bg-gray-900 hover:bg-gray-700 transition-all duration-500"
          >
            {" "}
            <svg
              width="22"
        ...
        {isLoading ? (
          ...
          <div className=" p-5">
            <Image
              ...
              width={1400}
              height={250}
              src={data?.newsfeed?.photo}
              alt={data?.newsfeed?.title}
            />
            <hr className="mt-5" />
            <h1 className="text-white text-[18px] my-5">
              {data?.newsfeed?.title}
            </h1>
